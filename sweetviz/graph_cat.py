...
def plot_grouped_bars(
    ...
    **kwargs,
    ...
    locations_centered + usable_for_bars / 2.0
    ...
    def __init__(self, which_graph: str, to_process: FeatureToProcess):
        ...
        if which_graph == "mini":
            f, axs = plt.subplots(
                ...
                1,
                figsize=(
                    config["Graphs"].getfloat("cat_summary_graph_width"),
            ...
            height = config["Graphs"].getfloat("detail_graph_height_base") + config[
                "Graphs"
            ].getfloat("detail_graph_height_per_elem") * max(1, len(plot_data_series))
            ...
            f, axs = plt.subplots(
                1, 1, figsize=(config["Graphs"].getfloat("detail_graph_width"), height)
            )
        ...
        plot_data_series = (
            plot_data_series / sum_source
            if sum_source != 0.0
            else plot_data_series * 0.0
        )
        ...
        if to_process.compare is not None:
            ...
            matched_data_series = (
                matched_data_series / sum_compared
                if sum_compared != 0.0
                else matched_data_series * 0.0
            )
        ...
        if len(tick_names):
            if isinstance(tick_names[0], str):
                tick_names_for_labels_only = [
                    str(x).replace("$", r"\$") for x in tick_names
                ]
        # colors = ("r", "b")
        category_centers, bar_width = plot_grouped_bars(
            tick_names_for_labels_only,
            height_lists,
            cycle_colors,
            gap_percent,
        ...
        if to_process.source_target is not None:
            if to_process.predetermined_type_target == FeatureType.TYPE_NUM:
                ...
                for name in tick_names:
                    if name == OTHERS_GROUPED:
                        tick_average = to_process.source_target[
                            ~to_process.source.isin(names_excluding_others)
                        ].mean()
                    else:
                        tick_average = to_process.source_target[
                            to_process.source == name
                        ].mean()
                    ...
                    for name in tick_names:
                        if name == OTHERS_GROUPED:
                            tick_average = to_process.compare_target[
                                ~to_process.compare.isin(names_excluding_others)
                            ].mean()
                        else:
                            tick_average = to_process.compare_target[
                                to_process.compare == name
                            ].mean()
                ...
                for name in tick_names:
                    if name == OTHERS_GROUPED:
                        tick_num = sv_math.count_fraction_of_true(
                            to_process.source_target[
                                ~to_process.source.isin(names_excluding_others)
                            ]
                        )[0]
                    else:
                        tick_num = sv_math.count_fraction_of_true(
                            to_process.source_target[to_process.source == name]
                        )[0]
                ...
                if (
                    ...
                    for name in tick_names:
                        if name == OTHERS_GROUPED:
                            tick_num = sv_math.count_fraction_of_true(
                                to_process.compare_target[
                                    ~to_process.compare.isin(names_excluding_others)
                                ]
                            )[0]
                        else:
                            tick_num = sv_math.count_fraction_of_true(
                                to_process.compare_target[to_process.compare == name]
                            )[0]
        ...
        if which_graph == "mini":
            needed_pixels_padding = np.array(
                [14.0, (300 + 32), 14, 45]
