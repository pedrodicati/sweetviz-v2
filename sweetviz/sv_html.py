...
jinja2_env.filters["fmt_percent_parentheses"] = (
    sweetviz.sv_html_formatters.fmt_percent_parentheses
)
...
jinja2_env.filters["fmt_div_icon_missing"] = (
    sweetviz.sv_html_formatters.fmt_div_icon_missing
)
jinja2_env.filters["fmt_div_color_override_missing"] = (
    sweetviz.sv_html_formatters.fmt_div_color_override_missing
)
...
def load_layout_globals_from_config():
    ...
    general_globals["association_min_to_bold"] = config["General"].getfloat(
...
def set_summary_positions(dataframe_report):
    ...
    for feature in dataframe_report._features.values():
        render_index = (
            feature["order_index"]
            if dataframe_report._target is None
            else (feature["order_index"] + 1)
        )
        feature["summary_pos"] = render_index * config["Layout"].getint(
            "summary_spacing"
        )
...
def generate_html_detail(dataframe_report):
    ...
    for feature in all_detail:
        ...
        if (
            feature["type"] == FeatureType.TYPE_CAT
            or feature["type"] == FeatureType.TYPE_BOOL
        ):
            feature["html_detail"] = generate_html_detail_cat(
                feature, compare_dict, dataframe_report
            )
        elif feature["type"] == FeatureType.TYPE_NUM:
            feature["html_detail"] = generate_html_detail_numeric(
                feature, compare_dict, dataframe_report
            )
        elif feature["type"] == FeatureType.TYPE_TEXT:
            feature["html_detail"] = generate_html_detail_text(
                feature, compare_dict, dataframe_report
            )
...
def generate_html_dataframe_page(dataframe_report):
    ...
    dataframe_report.page_height = 160 + (
        dataframe_report.num_summaries
        * (config["Layout"].getint("summary_height_per_element"))
    )
...
def create_summary_numeric_group_data(feature_dict: dict, compare_dict: dict):
    ...
    group_1 = list()
    group_1.append(
        {"name": "MAX", "value": stats["max"], "compare_value": get_compare("max")}
    )
    group_1.append(
        {
            "name": "95%",
            "value": stats["perc95"],
            "compare_value": get_compare("perc95"),
        }
    )
    group_1.append(
        {"name": "Q3", "value": stats["perc75"], "compare_value": get_compare("perc75")}
    )
        ...
        group_1.append(
            {
                "name": "AVG",
                "value": stats["mean"],
                "compare_value": get_compare("mean"),
            }
        )
        group_1.append(
            {
                "name": "MEDIAN",
                "value": stats["perc50"],
                "compare_value": get_compare("perc50"),
            }
        )
    else:
        group_1.append(
    ...
    group_1.append(
        {"name": "Q1", "value": stats["perc25"], "compare_value": get_compare("perc25")}
    )
    group_1.append(
        {"name": "5%", "value": stats["perc5"], "compare_value": get_compare("perc5")}
    )
    group_1.append(
        {"name": "MIN", "value": stats["min"], "compare_value": get_compare("min")}
    )
    ...
    group_2.append(
        {
            "name": "RANGE",
            "value": stats["range"],
            "compare_value": get_compare("range"),
        }
    )
    group_2.append(
        {"name": "IQR", "value": stats["iqr"], "compare_value": get_compare("iqr")}
    )
    group_2.append(
        {"name": "STD", "value": stats["std"], "compare_value": get_compare("std")}
    )
    group_2.append(
        {
            "name": "VAR",
            "value": stats["variance"],
            "compare_value": get_compare("variance"),
        }
    )
    ...
    group_2.append(
        {
            ...
            "compare_value": get_compare("kurtosis"),
        }
    )
    group_2.append(
        {
            ...
            "compare_value": get_compare("skewness"),
        }
    )
    group_2.append(
        {"name": "SUM", "value": stats["sum"], "compare_value": get_compare("sum")}
    )
...
def generate_html_summary_numeric(feature_dict: dict, compare_dict: dict):
    ...
    if compare_dict is not None:
        if (
            np.isnan(feature_dict["stats"]["range"])
            and np.isnan(compare_dict["stats"]["range"]) == False
        ):
    ...
    def formatted_range(val):
        if isinstance(val, str):
            return ""
        return sweetviz.sv_html_formatters.fmt_smart_range(
            val, feature_dict["stats"]["range"]
        )
...
def generate_html_summary_text(feature_dict: dict, compare_dict: dict):
    ...
    if len(summary_list) == max_text_rows:
        ...
        if compare_dict is not None:
            ...
            cur_count = sum(
                (
                    row_data["count_compare"].number
                    if row_data.get("count_compare")
                    else 0
                )
        ...
        if row["count"].number > 0 or (
            row.get("count_compare") and row.get("count_compare").number > 0
        ):
...
def generate_html_detail_numeric(
    feature_dict: dict, compare_dict: dict, dataframe_report
):
    ...
    if dataframe_report._associations is not None:
        ...
        categorical = {
            ...
            if dataframe_report.get_type(k) == FeatureType.TYPE_BOOL
            or dataframe_report.get_type(k) == FeatureType.TYPE_CAT
        }

        # Sort & get top
        max_num = config["Detail_Stats"].getint("max_num_top_associations")
        numerical = sorted(
            numerical.items(), key=cmp_to_key(cmp_assoc_values), reverse=True
        )[:max_num]
        categorical = sorted(categorical.items(), key=itemgetter(1), reverse=True)[
            :max_num
        ]

        # Set who's the target, for highlighting
        if dataframe_report._target is not None:
            numerical = add_is_target_or_not(
                numerical, dataframe_report._target["name"]
            )
            categorical = add_is_target_or_not(
                categorical, dataframe_report._target["name"]
            )
    else:
        max_num = None
        numerical = None
...
def generate_html_detail_cat(feature_dict: dict, compare_dict: dict, dataframe_report):
    ...
    detail_layout = dict()
    detail_layout["graph_y"] = config["Layout"].getint("cat_detail_graph_y")
    detail_layout["breakdown_y"] = (
        detail_layout["graph_y"]
        + feature_dict["detail_graphs"][0].size_in_inches[1] * 100
        + config["Layout"].getint("cat_detail_breakdown_y_offset")
    )
    ...
    # 37px->6 = 6.2
    # 216px->44 = 4.9
    cur_x = longest_width + config["Layout"].getint(
    ...
    feature_dict["detail"]["detail_count"] = feature_dict["detail"]["full_count"][
        :max_rows
    ]
    ...
    if dataframe_report._associations is not None:
        ...
        influencing = {
            ...
            if (
                dataframe_report.get_type(k) == FeatureType.TYPE_BOOL
                or dataframe_report.get_type(k) == FeatureType.TYPE_CAT
            )
            and k != feature_name
        }
        ...
        # Sort & get top
        max_num = config["Detail_Stats"].getint("max_num_top_associations")
        influencing = sorted(influencing.items(), key=itemgetter(1), reverse=True)[
            :max_num
        ]
        influenced = sorted(influenced.items(), key=itemgetter(1), reverse=True)[
            :max_num
        ]
        corr_ratio = sorted(corr_ratio.items(), key=itemgetter(1), reverse=True)[
            :max_num
        ]

        # Set who's the target, for highlighting
        if dataframe_report._target is not None:
            influencing = add_is_target_or_not(
                influencing, dataframe_report._target["name"]
            )
            influenced = add_is_target_or_not(
                influenced, dataframe_report._target["name"]
            )
            corr_ratio = add_is_target_or_not(
                corr_ratio, dataframe_report._target["name"]
            )
    else:
        influencing = None
        influenced = None
