...
def do_stats_numeric(series: pd.Series, updated_dict: dict):
    ...
    for percentile, value in (
        series.quantile([0.95, 0.75, 0.50, 0.25, 0.05]).to_dict().items()
    ):
...
def do_detail_numeric(
    series: pd.Series, counts: dict, counts_compare: dict, updated_dict: dict
):
    ...
    min_values = pd.DataFrame(
        counts["value_counts_without_nan"].sort_index(ascending=True).head(num_to_show)
    )
    max_values = pd.DataFrame(
        counts["value_counts_without_nan"].sort_index(ascending=False)
    ).head(num_to_show)
    ...
    for frequent, min_value, max_value in zip(
        frequent_values.itertuples(), min_values.itertuples(), max_values.itertuples()
    ):
        ...
        detail["frequent_values"].append(
            (
                frequent[0],
                NumWithPercent(frequent[1], total_num),
        ...
        detail["min_values"].append(
            (
                min_value[0],
                NumWithPercent(min_value[1], total_num),
        ...
        detail["max_values"].append(
            (
                max_value[0],
                NumWithPercent(max_value[1], total_num),
...
def analyze(to_process: FeatureToProcess, feature_dict: dict):
    ...
    if to_process.is_target():
        feature_dict["html_summary"] = sv_html.generate_html_summary_target_numeric(
