...
class GraphLegend(sweetviz.graph.Graph):
    def __init__(self, dataframe_report):
        ...
        fig = plt.figure(
            figsize=(
                config["Graphs"].getfloat("legend_width"),
                config["Graphs"].getfloat("legend_height"),
            )
        )
        ...
        axs.add_patch(
            patches.Rectangle(
                to_fractionsxy(gfx_x_source, gfx_bar_y),
                bar_size[0] * scale[0],
                bar_size[1] * scale[1],
                facecolor=cycle_colors[0],
            )
        )
        ...
        if dataframe_report.compare_name:
            axs.add_patch(
                patches.Rectangle(
                    to_fractionsxy(gfx_x_compare, gfx_bar_y),
                    bar_size[0] * scale[0],
                    bar_size[1] * scale[1],
        ...
        if dataframe_report.get_target_type() is not None:
            axs.add_line(
                matplotlib.lines.Line2D(
                    to_fraction_seq(
                        [gfx_x_source, gfx_x_source + bar_size[0]], scale[0]
                    ),
                    to_fraction_seq([gfx_line_y, gfx_line_y], scale[1]),
                    lw=(1),
            ...
            if dataframe_report.compare_name and dataframe_report._target.get(
                "compare"
            ):
                axs.add_line(
                    matplotlib.lines.Line2D(
                        to_fraction_seq(
                            [gfx_x_compare, gfx_x_compare + bar_size[0]], scale[0]
                        ),
                        to_fraction_seq([gfx_line_y, gfx_line_y], scale[1]),
                        lw=(1),
