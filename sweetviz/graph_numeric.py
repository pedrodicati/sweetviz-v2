...
class GraphNumeric(sweetviz.graph.Graph):
    def __init__(self, which_graph: str, to_process: FeatureToProcess):
        if config["Graphs"].getint("use_boxplots") != 0:
            self.boxplot(which_graph, to_process)
        else:
            self.histogram(which_graph, to_process)

    def boxplot(self, which_graph: str, to_process: FeatureToProcess):
    ...
    def histogram(self, which_graph: str, to_process: FeatureToProcess):
        if to_process.is_target() and which_graph == "mini":
            styles = ["graph_base.mplstyle", "graph_target.mplstyle"]
        else:
            styles = ["graph_base.mplstyle"]
